...
import { Reply, Trash2, Trash, Paperclip, X, MoreHorizontal, Edit2 } from "lucide-react";
...
const getMessageDateString = (dateStr) => {
    if (!dateStr) return "";
...
function MassangerSection({ selectedUser, selectedConversationId, onConversationChanged, onConversationStarted, onOpenPost, compactHeader = false }) {
            ...
            socket.onmessage = (ev) => {
                try {
                    ...
                    if (data?.type === 'new_message' && data.payload) {
                        ...
                        setMessages((current) => {
                            // If message already exists, replace it (handles edits and deletions!)
                            if (current.some((m) => m.id === newMsg.id)) {
                                return current.map((m) => m.id === newMsg.id ? newMsg : m);
                            }
    ...
    const handleSendMessage = async () => {
        ...
        if ((!content && !attachmentFile) || !selectedUser || sending) {
            return;
        }
        ...
        try {
            ...
            if (editingMessage) {
                const response = await fetch(`${API_BASE}/api/chat/messages/${editingMessage.id}/edit/`, {
                    method: "POST",
                    headers: {
                ...
                setMessages((currentMessages) =>
                    currentMessages.map((m) => m.id === editingMessage.id ? updatedMessage : m)
                );
                setDraft("");
            ...
            // If there's no conversation ID, start it on the backend first!
            if (!currentConversationId) {
                const startResponse = await fetch(`${API_BASE}/api/chat/conversations/start/`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: "Token " + token,
                ...
                if (typeof onConversationStarted === "function") {
                    onConversationStarted(currentConversationId, selectedUser);
                }
            }
    ...
    const canSend = Boolean(selectedUser && (draft.trim() || attachmentFile));
    ...
    const getImageValue = (message) => {
        const image = message?.image_url || message?.image || "";
        if (!image) return "";
        return image.startsWith("http") ? image : `${API_BASE}${image}`;
    };

    const getVideoValue = (message) => {
        const video = message?.video_url || message?.video || "";
        if (!video) return "";
        return video.startsWith("http") ? video : `${API_BASE}${video}`;
    };
    ...
    return (
        <main className={`w-full h-full flex flex-col min-h-0 ${compactHeader ? "p-0" : "p-2"}`}>
                                    ...
                                    <React.Fragment key={message.id}>
                                        {showDateSeparator && (
                                            <div className="flex justify-center my-4">
                                                <span className="bg-zinc-900 border border-zinc-800 text-zinc-400 text-xs px-3 py-1 rounded-full shadow-sm">
                                                    {getMessageDateString(message.created_at)}
                                                </span>
                                            </div>
                                        )}
                                        <div className={`relative group flex ${isOwn ? "justify-end" : "justify-start"} items-center gap-2 w-full`}>
                                            {isOwn && (
                                                <button
                                                    type="button"
                                                    onClick={() => setMenuMessage(message)}
                                                    className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-full hover:bg-zinc-800 text-zinc-400 hover:text-white shrink-0"
                                            ...
                                            >
                                                {message.reply_to ? (
                                                    <div className="mb-2 pl-2.5 py-0.5 text-xs text-zinc-400 bg-black/25 rounded-r-md">
                                                        Replying to <Link to={`/profile/${message.reply_to.sender_username}`} className="underline text-emerald-400 font-medium">@{message.reply_to.sender_username}</Link>: {message.reply_to.content}
                                                    </div>
                                                ) : null}
                                                ...
                                                <div className="mt-1 flex items-center justify-between gap-1 text-[10px] text-zinc-300/70">
                                        ...
                                        {isOwnLatestMessage ? (
                                            <div className="text-right text-[10px] text-zinc-500 pr-1 -mt-1.5 self-end">
            ...
            <div className={`bg-zinc-950/90 px-3 py-2.5 backdrop-blur-sm flex flex-col gap-2 ${compactHeader ? "border-t border-zinc-800" : "border-l border-r border-b border-zinc-800"}`}>
                {replyToMessage ? (
                    <div className="flex items-center justify-between gap-3 rounded-2xl  bg-zinc-900/80 px-3 py-2 text-xs text-zinc-300">
                ...
                <div className="flex items-center gap-3 rounded-full   px-3 py-2.5 ">
                <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    disabled={!selectedUser || sending}
                    className="rounded-full border border-zinc-700/80 p-2.5 text-zinc-300 transition hover:border-zinc-500 hover:bg-white/5 hover:text-white disabled:opacity-60 shadow-[0_2px_4px_rgba(0,0,0,0.2)] hover:scale-105 active:scale-95 shrink-0"
                ...
                <input
                    ...
                    className="flex-1 rounded-full border border-zinc-800 bg-zinc-950 px-4 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-500 focus:border-emerald-450 focus:outline-none focus:ring-1 focus:ring-emerald-400/25 shadow-inner"
                />
                <button
                    type="button"
            ...
            {confirmDialog.open ? (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
                    <div className="w-full max-w-md rounded-2x bg-zinc-950 p-5">
            ...
            {menuMessage ? (
                ...
                >
                    <div 
                        className="w-full max-w-xs rounded-2xl bg-zinc-950 border border-zinc-800 p-3 shadow-xl animate-in fade-in zoom-in-95 duration-155"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className="text-zinc-500 text-[11px] px-3 pb-2 border-b border-zinc-900 truncate">
                            Message: "{menuMessage.content}"
                        </div>
                        <div className="flex flex-col gap-1 mt-2">
                            {!menuMessage.deleted_for_everyone && (
                                <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="w-full flex items-center gap-3 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-900 rounded-lg transition-colors text-left"
                                >
                                    <Reply size={15} />
                                    <span>Reply</span>
                                </button>
                            ...
                            {menuMessage.is_mine && !menuMessage.deleted_for_everyone && (
                                <button
                                    type="button"
                                    onClick={() => {
                                        setEditingMessage(menuMessage);
                                        setDraft(menuMessage.content);
                                        setMenuMessage(null);
                                    }}
                                    className="w-full flex items-center gap-3 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-900 rounded-lg transition-colors text-left"
                                >
                            ...
                            <button
                                type="button"
                                onClick={() => {
                                    askDeleteForMe(menuMessage.id);
                                    setMenuMessage(null);
                                }}
                                className="w-full flex items-center gap-3 px-3 py-2 text-sm text-red-400 hover:bg-red-950/20 rounded-lg transition-colors text-left"
                            >
                                <Trash2 size={15} />
                                <span>Delete for Me</span>
                            </button>

                            {menuMessage.can_delete_for_everyone && (
                                <button
                                    type="button"
                                    onClick={() => {
                                        askDeleteForEveryone(menuMessage.id);
                                        setMenuMessage(null);
                                    }}
                                    className="w-full flex items-center gap-3 px-3 py-2 text-sm text-red-500 hover:bg-red-950/40 rounded-lg transition-colors text-left"
                                >
                                    <Trash size={15} />
