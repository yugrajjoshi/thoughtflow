...
function Profile() {
    ...
    const toMediaUrl = (value) => {
        if (!value) return "";
        return value.startsWith("http") ? value : `http://127.0.0.1:8000${value}`;
    };
            ...
            <div className="flex flex-col min-h-screen w-2/3 text-white border-zinc-500 border-l-[0.5px] border-r-[0.5px] relative">
               ...
               <div className="flex flex-col w-full h-[80%]">
                <div className="flex w-full h-[35%]  border-white text-white items-center justify-center">
                                        {bannerPicture ? (
                                            <img src={bannerPicture} className="w-full h-full object-cover" />
                                        ) : (
                ...
                <div className="border-b border-white w-full h-[80%] border-0.5">
                    <div className="flex flex-col w-full h-full mt-20 pl-5 pr-5 pt-5 gap-2">
                        <div className="flex items-start justify-between w-full">
                            ...
                            <button
                                className="px-4 py-2 rounded-3xl bg-zinc-700 hover:bg-zinc-600 text-white"
                                onClick={openEdit}
                            >
                                Edit Profile
                            </button>
                        ...
                        <p className="text-lg text-gray-400">{userData?.bio}</p>
                                                        <div className="flex fel-row gap-40"><h3 className=" text-zinc-500 pt-10" ><CalendarClockIcon className="w-5 h-5 inline mr-2" />  join date {formatDate(userData?.date_joined || userData?.created_at)}</h3>
            ...
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
                <form onSubmit={handleEditSubmit} className="w-[92%] max-w-2xl bg-zinc-900 border border-zinc-700 rounded-2xl p-5 text-white">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button type="button" onClick={() => setIsEditOpen(false)} className="text-zinc-400 hover:text-white">Close</button>
                    </div>
                    ...
                    <div className="flex justify-end mt-4 gap-3">
                        <button type="button" onClick={() => setIsEditOpen(false)} className="px-4 py-2 rounded-3xl bg-zinc-700 hover:bg-zinc-600">Cancel</button>
